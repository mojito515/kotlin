...
fun test() {
    val s : Set<Int> = <!OI;TYPE_INFERENCE_EXPECTED_TYPE_MISMATCH!><!NI;TYPE_MISMATCH!><!NI;TYPE_MISMATCH!>newList()<!><!><!>
    use(s)
}
...
fun test1(outA: Out<A>, inB: In<B>) {
    ...
    val b: Two<A, C> = <!OI;TYPE_INFERENCE_EXPECTED_TYPE_MISMATCH!><!NI;TYPE_MISMATCH!><!NI;TYPE_MISMATCH!>foo(outA, inB)<!><!><!>
    use(b)
}
